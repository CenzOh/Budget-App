...
#adding up all amounts of items to acquire total.
  ...
  def withdraw(self, amount, description=""):
    if(self.check_funds(amount)): #see if we have enough money in acc, if so, withdraw
      self.ledger.append({"amount": -amount, "description": description})
      return True
    return False
  ...
  def check_funds(self, amount):
    if(self.get_balance() >= amount): #ensure we have more moeny in acc than what we're asking for the transaction
      return True
    return False
